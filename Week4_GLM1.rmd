...
https://github.com/michellebyrne1/MonashHonoursStatistics/blob/master/GLM1_Michelle.rmd
...
visreg(m, xvar = "stress", partial = FALSE, rug = FALSE, gg = TRUE) +
  theme_pubr() +
  annotate("text", x = 22, y = 8, label = "b = 0.XX, p < .0001", size = 4) + 
  ggtitle("Linear regression of neuroticism on stress",
          subtitle = "Shaded region shows 95% confidence interval.")
...
transformation of the outcome (although you wouldn't do this if the normality actually 
looks ok - but let's just do it as an example anyway.
...
There 
`r ifelse(coef(summary(m3))["sex1", "Pr(>|t|)"] < .05, "was", "was not")`
...
`r sprintf("%0.2f", round(coef(m3)[["sex1"]], 2))` 
[95% CI `r sprintf("%0.2f - %0.2f", round(confint(m3)["sex1", 1], 2), round(confint(m3)["sex1", 2], 2))`]
higher neuroticism scores, 
`r formatPval(coef(summary(m3))["sex1", "Pr(>|t|)"], d = 2, includeP = TRUE)`.
...
and sex on neuroticism. Note that when there are interactions,
...
on neuroticism, such that for male participants (i.e., when SexDummy =
...
Overall we can interpret that there is no statistically significnat
