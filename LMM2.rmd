...
two random varaibles, say age and stress, correlate with each other,
...
The variances are on the diagnonal and the covariance (the
...
"point" can be unidimensal or multidimensional and if multidimensional
...
close match, it indicates the data on which the Mahalnobis distances
were calculated were MVN. Thus, we can use Mahalnobis distances to
...
univariate normal distributiones (blue dashed lines) for the random
...
univariate random effect distribution plots can be interpretted as
...
the random effects by ID follow a MVN or not by using the mahalnobis
...
In practice, I may not atually make any changes here. For the sake of
...
the dataset are extrme and should / could be dropped.
...
`r sprintf("[%0.2f, %0.2f]", m2.ci[5, 1], m2.ci[5, 2])`.
...
We have talked before some about convergance, where algorithms iterate
...
`energy` and look at a fixed and random slope LMM predicting stresss
from energy.
...
## use the control function to picka  different algorithm
...
very cautious about interpretting a model that did not converge.
...
predicted to be, but does indiate what, on average, the prediction
...
fixed effects part of the model. They are interpretted nearly
...
Here we can see that `Wmood` has the class haven labelled. As this is
...
the trouble. `visreg()` may not know how to work with haven labelled
...
influence the results and the indvidiaul differences in both level of
...
2. Since we know `visreg()` does not like haven labelled type
   variables, check if any of your stress variables or the `energy`
   variable is haven labelledand if so, convert them to numeric.
