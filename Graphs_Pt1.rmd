...
The breaks for positive affect are useful but probably printed in too
much precision. We can use the `round()` function to reduce this
precision and clean up the numbers.

```{r, fig.width = 6, fig.height = 5, fig.cap = "Scatter plot with a lot of data reducing overplotting with rounding for positive affect"}
...
third variable changes slowly with x or y.
...
indicate higher density (i.e., more observations). 
...
with non missing support values and creates a line plot.
