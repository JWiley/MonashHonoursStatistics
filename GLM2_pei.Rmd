...
author: "Joshua F. Wiley, with updates from Pei Hwa Goh"
date: "`r Sys.Date()`"
output:
  tufte::tufte_html: 
    toc: true
...
## compute a neuroticism variable if you haven't already done so
db [, N1 := 6- BFI_N1r]
db[, neuroticism:= rowMeans(.SD, na.rm = TRUE),
.SDcols = c("N1", "BFI_N2")]

## create some binary variables
db[, StressHigh := as.integer(stress >= 2.51)]
db[, SelfesteemHigh := as.integer(selfesteem >= 3.751)]
...
only 11 people reported both high self esteem and high stress.
...
relative risk of 0.345 of being high stress as do people with low self
esteem. In a simple case like this model, that directly means that
people with high self esteem have 0.345 times the probability of being
