...
associated with the estimate of our parameter(s) of interest[^mnar]

[^mcar]: When data are missing completely at random (MCAR), listwise
deletion will yield unbiased estimates of the true parameter(s) if the
data had not been missing.

[^mar]: When data are missing at random (MAR), 
...
[^mnar]: When data are missing not at random (MNAR), 
...
[^aggrplot]: The left hand side of this missing data plot shows the 
...
indicates present data. Red indicates missing data. About 56% of
...
about 21% of people. The next most common pattern is having all
variable but to be missing positive affect, with about 14% of people.
...
so there is only one boxplot for negative affect[^marginplot].

[^marginplot]: The margin plot starts with a scatter plot. Blue dots
...
t-tests or chi-square tests[^missingtests].

[^missingtests]: These tests tell us whether values on one variable
...
with additional iterations[^convergence].

[^convergence]: The convergence diagnostics show estimated values of
...
versus observed data. Imputed data are in red[^midiagnostics].

[^midiagnostics]: These diagnostic plots do not necessarily tell you
...
[^pooloutput]: The pooled output has several pieces:</br>
**estimate**: the pooled, average regression coefficients;</br>
**ubar**: the average variance (based on the squared standard errors)
  associated with sampling variation, we called it $\bar{V}$;</br>
**b**: the between imputation variance; </br>
**t**: the total variance incorporating sampling variation, between
  imputation variance, and simulation error;</br>
**dfcom**: the degrees of freedom for the complete data analysis (i.e.,
  if there had not been missing data);</br>
**df**: the estimated residual degrees of freedom for the model, taking
  into account missingness;</br>
**riv**: the relative increase variance due to missing data;</br>
**lambda** or $\lambda$: the proportion of total variance due to
  missing data, that is: $\lambda = \frac{B + B/m}{T}$;</br>
...
output in this case.</br>
**estimate**: the pooled, average regression coefficients;</br>
**std.error**: the standard error of the estimates incorporating all
  three sources of MI error, specifically: $se = \sqrt{T}$;</br>
**statistic**: $\frac{estimate}{std.error}$;</br>
**df**: the estimated degrees of freedom incorporating uncertainty due
  to missing data, combined with the statistic to calculate p-values;</br>
**p.value**: the p-value, the probability of obtaining the estimate or
  larger in the sample given that the true population value was zero.</br>
**2.5%**: the lower limit of the 95% confidence interval;</br>
...
complete case analysis[^truthnote].

[^truthnote]: In general we do not have the true estimates. In this
